...
// 局所排気装置・コンプレッサの「管理番号：」は元のテンプレートに欄が無かったため、
// Setup.gsのaddManagementNumberLabels()で1回だけ追加してから使う。
// リフトのみ、該当する設備名・管理番号らしき欄がテンプレート自体に
// 見当たらなかったため対象外。
const HEADER_LABEL_FIELDS = {
  '溶接機': [['管理番号：', '管理番号'], ['部署：', '部署'], ['場所：', '設置場所'], ['責任者：', '責任者']],
  'クレーン': [['クレーンＮＯ：', '管理番号']],
  '局所排気装置': [['設備名：', '設備名'], ['担当：', '責任者'], ['管理番号：', '管理番号']],
  'コンプレッサ': [['工　場　名', '設置場所'], ['点　検　者', '責任者'], ['管理番号：', '管理番号']]
};
